    ...
    for p in points:
        ...
        for sock in ALLOWED_SOCKETS:
            ...
            if count > 0:
                ...
                socket_outputs[sock] = max(socket_outputs[sock], output)

            total_sockets += count
            total_kw += count * output

    return dict(socket_counts), dict(socket_outputs), total_sockets, round(total_kw, 2)
...
for way_id, way_data in data.items():
    ...
    socket_counts, socket_outputs, total_sockets, total_kw = extract_sockets_and_kw(valid_points)
    ...
    for child_point in valid_points:
        ...
        child_socket_counts, child_socket_outputs, child_total_sockets, child_total_kw = extract_sockets_and_kw(cprops)
        ...
        for k, v in cprops.items():
            if k.endswith(':output'):
                ...
                if base_key in ALLOWED_SOCKETS:
                    ...
                    child_socket_outputs[base_key] = num_val
...
for point in data.get('unassigned_points', []):
    ...
    socket_counts, socket_outputs, total_sockets, total_kw = extract_sockets_and_kw(pprops)
...
fieldnames = [
    ...
    'access', 'payment:cards', 'opening_hours', 'network',
    'total_sockets', 'total_kw', 'flag_missing_tag',
    'overflow', 'overflow_power', 'adjusted_kw'
]

# add column for count and column for power
for sock in sorted(all_socket_types):
    fieldnames.append(sock)           # number of sockets
    fieldnames.append(f"{sock}:kw")   # power per socket


with open('stations_2.csv', 'w', newline='', encoding='utf-8') as csvfile:
    writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
    writer.writeheader()
    for row in rows:
        writer.writerow(row)

print(f"CSV file 'stations_2.csv' created with {len(rows)} entries. Socket types: {sorted(all_socket_types)}")
